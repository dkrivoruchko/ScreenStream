...
internal class NetworkHelper(private val context: Context) {
    ...
    private companion object {
        private val INTERFACE_PATTERNS = mapOf(
            RtspSettings.Values.INTERFACE_WIFI to listOf("wlan\\d", "ap\\d", "wigig\\d", "softap\\.?\\d").map { it.toRegex() },
            RtspSettings.Values.INTERFACE_MOBILE to listOf("rmnet.*", "ccmni.*", "usb\\d").map { it.toRegex() },
            RtspSettings.Values.INTERFACE_ETHERNET to listOf("eth\\d", "en\\d", "lan\\d").map { it.toRegex() },
            RtspSettings.Values.INTERFACE_VPN to listOf("tun\\d", "tap\\d", "ppp\\d", "vpn").map { it.toRegex() }
        )
    ...
    private val enhancedInterfacePatterns: Map<Int, List<Regex>> by lazy {
        INTERFACE_PATTERNS.mapValues { (type, patterns) ->
            if (type == RtspSettings.Values.INTERFACE_WIFI) {
    ...
    fun getNetInterfaces(
        ...
        val netInterfaces = mutableListOf<RtspNetInterface>()

        if (addressFilter != RtspSettings.Values.ADDRESS_LOCALHOST) {
        ...
        if (netInterfaces.isEmpty() && (enableIpv4 || enableIpv6) && addressFilter != RtspSettings.Values.ADDRESS_LOCALHOST) {
    ...
    private fun discoverViaConnectivityManager(
        ...
        enableIpv4: Boolean,
        enableIpv6: Boolean,
        sink: MutableList<RtspNetInterface>
    ) {
        connectivityManager?.allNetworks?.forEach { network ->
            ...
            val transportMask = when {
                caps.hasTransport(NetworkCapabilities.TRANSPORT_WIFI) -> RtspSettings.Values.INTERFACE_WIFI
                caps.hasTransport(NetworkCapabilities.TRANSPORT_CELLULAR) -> RtspSettings.Values.INTERFACE_MOBILE
                caps.hasTransport(NetworkCapabilities.TRANSPORT_ETHERNET) -> RtspSettings.Values.INTERFACE_ETHERNET
                caps.hasTransport(NetworkCapabilities.TRANSPORT_VPN) -> RtspSettings.Values.INTERFACE_VPN
                else -> 0
            }
    ...
    private fun passesAddressFilter(addr: InetAddress, @AddressMask filter: Int): Boolean {
        if (filter == RtspSettings.Values.ADDRESS_ALL) return true
        ...
        return when {
            addr.isLoopbackAddress -> (filter and RtspSettings.Values.ADDRESS_LOCALHOST) != 0
            isPrivate -> (filter and RtspSettings.Values.ADDRESS_PRIVATE) != 0
            else -> (filter and RtspSettings.Values.ADDRESS_PUBLIC) != 0
        }
    }
    ...
    private fun toMjpegNetInterface(address: InetAddress, transportMask: Int): RtspNetInterface {
        ...
        val addressLabel = when {
            address.isLoopbackAddress -> context.getString(R.string.rtsp_label_loopback)
            address is Inet6Address && isULA(address) -> context.getString(R.string.rtsp_label_ipv6_ula)
            address.isSiteLocalAddress -> context.getString(R.string.rtsp_label_ipv4_lan)
            address is Inet6Address -> context.getString(R.string.rtsp_label_public_ipv6)
            else -> context.getString(R.string.rtsp_label_public_ipv4)
        }

        val label = if (address.isLoopbackAddress) {
            context.getString(R.string.rtsp_label_interface, addressLabel, if (address is Inet6Address) "IPv6" else "IPv4")
        } else {
            context.getString(R.string.rtsp_label_interface, interfaceLabel, addressLabel)
        }

        return RtspNetInterface(label, address)
    }
}
