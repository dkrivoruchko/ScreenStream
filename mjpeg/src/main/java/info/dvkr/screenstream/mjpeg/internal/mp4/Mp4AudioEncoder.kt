...
import info.dvkr.screenstream.mjpeg.internal.audio.MjpegInternalAudioSource
import info.dvkr.screenstream.mjpeg.internal.audio.MjpegMicrophoneSource
import info.dvkr.screenstream.mjpeg.internal.audio.MjpegMixAudioSource
import kotlinx.coroutines.CoroutineDispatcher
import java.util.concurrent.ArrayBlockingQueue

internal class Mp4AudioEncoder(
    private val codecInfo: Mp4AudioEncoderInfo,
    private val generation: Long,
    private val onAudioConfig: (Mp4AudioConfig) -> Unit,
    private val onAudioPacket: (Mp4AudioPacket) -> Unit,
    ...
    internal fun prepare(
        ...
        runCatching {
            synchronized(encoderLock) {
                ...
                val format = MediaFormat.createAudioFormat(AAC_MIME_TYPE, audioParams.sampleRate, channelCount).apply {
                ...
                handlerThread = HandlerThread("Mp4AudioEncoderHandler", Process.THREAD_PRIORITY_AUDIO).apply { start() }
                ...
                onAudioConfig(
                    Mp4AudioConfig(
                        ...
                        audioSpecificConfig = Mp4AudioEncoderUtils.aacLcAudioSpecificConfig(audioParams.sampleRate, channelCount)
                    )
                )
            }
        }.onFailure { cause ->
    ...
    internal fun start(): Unit = synchronized(encoderLock) {
        if (audioSource == null || currentState != State.PREPARED) return
        audioEncoder?.start()
        audioSource!!.start()
    ...
    private fun createCodecCallback(): MediaCodec.Callback = object : MediaCodec.Callback() {
        ...
        override fun onOutputBufferAvailable(codec: MediaCodec, index: Int, info: MediaCodec.BufferInfo) {
            runCatching {
                val packet = synchronized(encoderLock) {
                    ...
                    val durationSamples = durationSamplesQueue.poll() ?: 1024
        ...
        override fun onOutputFormatChanged(codec: MediaCodec, format: MediaFormat) {
            ...
            val data = ByteArray(csd.remaining()).also { csd.get(it) }
            onAudioConfig(Mp4AudioConfig(AAC_SAMPLE_RATE, channelCount, data))
        }
    }
}
