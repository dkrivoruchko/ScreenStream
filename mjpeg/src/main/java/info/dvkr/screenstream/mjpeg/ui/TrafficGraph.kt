...
public class TrafficGraph @JvmOverloads constructor(
    ...
    private val gradientPaint = Paint().apply { style = Paint.Style.FILL }
    ...
    public fun setDataPoints(points: List<Pair<Long, Float>>) {
        dataPoints = points.sortedBy { it.first }.map { DataPoint.fromPair(it) }.toTypedArray()

        calculatePositions()

        gradientPaint.shader = LinearGradient(0f, paddingTop.toFloat(), 0f, zeroY, gradientColors, null, Shader.TileMode.CLAMP)
