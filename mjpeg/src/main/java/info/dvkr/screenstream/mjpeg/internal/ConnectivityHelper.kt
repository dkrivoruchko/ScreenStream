...
internal sealed class ConnectivityHelper {
    internal companion object {
        internal fun getInstance(context: Context): ConnectivityHelper =
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) NougatConnectivityHelper(context)
            else EmptyConnectivityHelper()
    }

    protected val connectionChangEventFlow =
        MutableSharedFlow<Network>(extraBufferCapacity = 8, onBufferOverflow = BufferOverflow.DROP_OLDEST)
    ...
    private class NougatConnectivityHelper(context: Context) : ConnectivityHelper() {
        ...
        private val networkCallback = object : ConnectivityManager.NetworkCallback() {
            override fun onAvailable(network: Network) {
                XLog.v(this@NougatConnectivityHelper.getLog("onAvailable", "Network: $network"))
                connectionChangEventFlow.tryEmit(network)
            }

            override fun onLost(network: Network) {
                XLog.v(this@NougatConnectivityHelper.getLog("onLost", "Network: $network"))
                connectionChangEventFlow.tryEmit(network)
            }
        }
        ...
        override fun startListening(coroutineScope: CoroutineScope, onConnectionChanged: () -> Unit) {
            ...
            connectionChangEventFlow.conflate()
                ...
                .onEach {
                    XLog.v(this@NougatConnectivityHelper.getLog("onEach", "Network: $it"))
                    onConnectionChanged.invoke()
                    delay(250)
