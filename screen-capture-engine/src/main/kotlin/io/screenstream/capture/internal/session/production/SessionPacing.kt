...
/**
 * Pure elapsed-time pacing. Fresh capture and output have independent grant histories. MaxFps distributes fractional
 * nanoseconds by carrying once per phase and never catches up missed grants. SamplingInterval gates only fresh capture.
 */
internal object PacingCalculator {
    ...
    private fun samplingIntervalFreshCapture(interval: Duration, nowNanos: Long, lastFreshGrantNanos: Long?): PacingDecision {
        ...
        else PacingDecision.Deferred(eligibleAt)
    }
    ...
    private fun maxFps(
        ...
        if (history != null) {
            ...
            if (nowNanos < eligibleAt) return PacingDecision.Deferred(eligibleAt)
        }
        val sum = phase + remainder
        val carry = if (sum >= fps) 1 else 0
        return PacingDecision.Eligible(sum - (carry * fps), quotient + carry.toLong())
    }
