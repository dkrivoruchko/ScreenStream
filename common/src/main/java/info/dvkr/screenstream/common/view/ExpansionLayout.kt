...
public class ExpansionLayout @JvmOverloads constructor(
    context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0
) : NestedScrollView(context, attrs, defStyleAttr) {

    public interface Listener {
        public fun onExpansionChanged(expansionLayout: ExpansionLayout, expanded: Boolean)
    }

    public interface IndicatorListener {
        public fun onStartedExpand(expansionLayout: ExpansionLayout, willExpand: Boolean)
    }
    ...
    public fun addIndicatorListener(listener: IndicatorListener?) {
        if (listener != null && indicatorListeners.contains(listener).not()) indicatorListeners.add(listener)
    }

    public fun removeIndicatorListener(listener: IndicatorListener?) {
    ...
    public fun collapse(animated: Boolean) {
    ...
    public fun expand(animated: Boolean) {
    ...
    public fun toggle(animated: Boolean) {
        if (isExpanded) {
            collapse(animated)
        } else {
            expand(animated)
