# Screen Capture Engine Design v35
...
>
> Revision v35 keeps the Kotlin-only API, atomic startup boundary, projection attachment/consumption split, generation-owned projection target model, startup arbiter, final commit-return handoff, provider-preparation isolation, delivery/admission semantics, first-plan render transform package, byte-exact encoder SPI, transactional encoded-sink discard semantics, correctness-first framework JPEG baseline, first ES2/framework-JPEG runtime path, and internal factory milestone boundary from earlier revisions. It clarifies the next design slice: built-in `CaptureMetricsProviders.*` are non-throwing latest-value providers with factory-specific bounds sources and explicit Activity/UI/display ownership semantics; valid runtime metric changes are authoritative inputs even when the engine must temporarily suspend output; partial `setParameters(...)` implementations reject unsupported retargeting with `ParameterUpdateUnavailable` without weakening the design-complete transaction contract; parameter-update commit uses one serialized resource/state generation gate; and repeated runtime hard-failure counters, reset boundaries, encoded-size exclusions, runtime GL watchdog severity, and public observability of failure exhaustion are now normative.

## Table of Contents

- [Goal and Non-Goals](#goal-and-non-goals)
- [Implementation Milestones and Release Completeness](#implementation-milestones-and-release-completeness)
...
## Implementation Milestones and Release Completeness
...
That milestone makes `CaptureMetricsProviders.*` usable as non-throwing, lazy, latest-value providers. It still does not imply that runtime reconfiguration, backend fallback, real-device validation, or release-complete default-engine behavior is finished.
...
`ScreenCaptureEngines.create(context)` is the public concrete construction entrypoint for the default Android implementation. The factory stores only application-safe context state internally and does not start capture, attach metrics-provider listeners, request projection consent, create a `MediaProjection`, or allocate GL/encoder resources. The implementation should store the application context or other application-safe state, not an `Activity`-lifetime object, because an engine instance may outlive the caller component that created it. Advanced dependency-injection wrappers may be built by the integrating app around the `ScreenCaptureEngine` interface, but this revision exposes one stable platform-module factory rather than a public builder.

An internal caller-supplied-metrics factory milestone may wire this factory before built-in metrics providers are implemented, provided that milestone remains gated and is not described as release-complete. A design-complete public artifact must not expose `CaptureMetricsProviders.*` methods that throw because they are unimplemented.

A single default `ScreenCaptureEngine` instance admits at most one non-terminal returned session at a time. A second `startSession(...)` call while an earlier session from the same engine is not terminal fails before projection attachment or consumption with `ScreenCaptureStartException(requiresFreshProjection = false, problem.kind = EngineSessionAlreadyActive)`. Engines are cheap enough that callers needing concurrent captures should create separate engine instances, each with its own active-session slot and engine-scoped guardrails. Sequential sessions on one engine are allowed after the previous session has reached a logical terminal boundary; heavy asynchronous cleanup may still be retiring resources, but old resources must remain fenced from the new startup.
...
`ProviderPreparationContext` is an engine-scoped internal service with session-scoped preparation and cleanup records. It is not local to one `startSession(...)` call and must not be closed in startup `finally` after a successful session, because active sessions and late provider results may still need isolated encoder cleanup. The service may be created lazily and may internally idle-shutdown when there are no active sessions, provider-preparation records, cleanup tasks, or quarantined workers, but the per-engine guardrails remain associated with the `ScreenCaptureEngine` instance. This revision does not add a public engine `close()` API; tests may use internal hooks to close engine-scoped services.

Provider preparation rules:

- Provider preparation uses the engine-owned `ProviderPreparationContext` with bounded admission and generation-bound plan-preparation-token fencing.
...
- A valid request that requires capture-target resize, `VirtualDisplay.resize(...)`, `VirtualDisplay.setSurface(...)`, or target replacement in an implementation slice that does not yet support retargeting is rejected with `ParameterUpdateUnavailable`. It is not `OutputPlanInvalid` because the request may be valid, and it is not a surface failure because the engine did not attempt the retargeting operation.
- `ParameterUpdateUnavailable` is a milestone/feature-boundary problem kind. Design-complete release behavior must implement supported retargeting transactions rather than treating valid retargeting updates as permanently unavailable.
- Same-target updates are allowed while output is `Running(Suspended)` only when the transaction can validate a coherent plan for the current capture geometry and runtime resources and either resume output to `Running(Active)` or maintain a deliberately validated suspended state. A partial implementation that cannot reason about suspended-state recovery must reject valid same-target updates with `ParameterUpdateUnavailable` rather than pretending they were applied.
- If requested parameters are invalid for the current geometry or caps, use `OutputPlanInvalid`, `OutputLimitsExceeded`, `EncodedSizeLimitExceeded`, or the more specific planning problem, regardless of implementation completeness.
...
Commit is one serialized generation gate. The normative ordering is:

```text
1. pause/admit no new producer scheduling for the old generation;
2. recheck terminal/session/generation gates;
3. install the candidate runtime resource owner and new output generation;
4. fence old runtime resources so late old work becomes stale;
5. publish public state/effective parameters for the new plan;
6. tryEmit diagnostic events such as OutputPlanApplied, CaptureTargetChanged, ReadbackModeChanged, or EncoderChanged;
7. enable scheduling for the new generation;
8. retire old resources asynchronously after active leases complete or become stale.
```

The public state must not advertise a plan whose runtime resources have not been installed. Conversely, no frame from the new generation may be published before the state/effective-parameters update for that generation is visible.

If commit wins before `stop()`, `close()`, projection stop, or fatal failure, `setParameters(...)` returns `Applied`, even if a terminal transition happens immediately after commit. If a terminal transition wins before commit, `setParameters(...)` returns `Rejected(problem)` when the caller is still active. Owner `stop()`/`close()` maps to a stopped-session problem; projection stop maps to the projection-stop problem; fatal failure maps to the terminal failure problem. If validation fails before commit, `setParameters(...)` returns `Rejected(problem)` and the previous active plan remains in use.
...
`CaptureMetrics` intentionally has no unavailable state. Built-in providers therefore use latest-valid semantics:

- a factory attempts to create a positive best-effort initial `CaptureMetrics` snapshot from its intended source or from an allowed fallback for that factory;
- a provider emits only positive effective metrics;
- invalid or unavailable running observations are suppressed and the last valid metrics remain in `metrics.value`;
- if a session attaches for startup and the provider cannot obtain a valid current or allowed fallback metrics snapshot for that factory, startup fails before projection callback attachment and projection consumption whenever possible with `MetricsUnavailableOrInvalid`;
- the design does not add an explicit unavailable metrics state in this revision.
...
| `fromDisplay(baseContext, display)` | Explicit display-bound provider for integrations that intentionally target a display. | Maximum/display-area bounds for that display when available, otherwise display-context resource metrics for that display. | Display-context resource metrics are an acceptable best-effort fallback. If the display is removed or becomes invalid, active sessions keep the last valid metrics and the provider suppresses invalid changes; future startups for that display fail with `MetricsUnavailableOrInvalid` rather than falling back to the default display. |
| `bestEffort(context)` | Service/application/default integration fallback. | Maximum window/display bounds from the safest available application/default-display context, because this factory is the closest built-in approximation for full-display MediaProjection bootstrap when no stable UI/window owner is available. | Falls back to application/resource display metrics when no better window/display metrics source is available. It may be less accurate on old APIs, external displays, desktop/freeform modes, or non-standard launch flows. |
...
- invalid running metrics are ignored, keep the last valid geometry, and emit `InvalidMetricsIgnored` when a public session exists;
...
and density. They do not by themselves make the default engine release-complete. Runtime
setParameters transactions, live resize/replan, backend fallback policy, transport integration,
and real-device validation remain separate completion gates.
```
...
- `ParameterUpdateUnavailable`: internal milestone rejection for unimplemented parameter transactions or valid updates requiring unsupported retargeting/recovery in a partial implementation. It is not normal design-complete public behavior.
...
Parameter-update tests must distinguish valid-but-unavailable retargeting (`ParameterUpdateUnavailable`) from invalid plans (`OutputPlanInvalid`/`OutputLimitsExceeded`) and must verify commit ordering: resource owner installed, public state published, then new-generation scheduling enabled.
...
- Android Context API reference: https://developer.android.com/reference/android/content/Context
- Android WindowManager / current window metrics API reference: https://developer.android.com/reference/android/view/WindowManager
- Android WindowMetrics API reference: https://developer.android.com/reference/android/view/WindowMetrics
