...
| Android API bands, metrics, MediaProjection and VirtualDisplay | [`AND-MET-001`](06-domain-android-capture-metrics.md#and-met-001--source-identity-and-observation-ownership)–[`AND-CAP-070`](06-domain-android-capture-metrics.md#and-cap-070--closed-alternatives) |
...
the receive-only last lane until all other lane receipts and external facts settle; a nonreturn retains that
...
| `ACC-21` | Callback and internal submission sides settle independently; exact `execute`/entry ordering selects busy/internal-terminal/callback behavior without double settlement. | `H-DL`, `H-OS`, `A-CL` | [`CTRL-200`](05-domain-controller-reconciliation.md#ctrl-200--policy-attempt-counter-and-observation-authority), [`DEL-HO-010`](12-domain-delivery-observation.md#del-ho-010--submission-entry-callback-and-classification), [`CORE-EXEC-1`](03-shared-runtime.md#core-exec-1--private-execute-endpoints) |
...
| T5 unsubscribe versus queued callback | An accepted Delivery task and unsubscribe race. Closure-first makes later Runnable entry inert and resolves the record; entry-first follows entered-callback settlement. If callback/Runnable return before outward `execute` settles, the handoff remains busy and unsubscribe still waits for that exact submission side. | `H-DL` |
